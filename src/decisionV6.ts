...
const moderateVolumeConfirmed = (input: DayTradeDecisionInput): boolean => {
  const price = input.metrics.currentPrice;
  if (price === null || price <= 0) return false;
  const atr = input.metrics.intradayAtr ?? Math.max(price * 0.002, 0.01);
...
const buildAdaptiveProbeLong = (
  ...
  const timingFactor = ADAPTIVE_LONG_RULE.alignedContinuationFactor;
