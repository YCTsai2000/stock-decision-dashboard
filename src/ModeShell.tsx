...
import { Activity, CandlestickChart, Gauge, LineChart } from 'lucide-react';
import App from './App';
import DayTradingAppV5 from './DayTradingAppV5';
import MuResearchPanel from './MuResearchPanel';

type AppMode = 'strategic' | 'daytrade' | 'mu';
  ...
  return (
    <>
      <div className="fixed top-2 left-1/2 -translate-x-1/2 z-[100] flex max-w-[calc(100vw-16px)] items-center gap-1 overflow-x-auto rounded-xl border border-slate-700/80 bg-slate-950/95 p-1 shadow-2xl backdrop-blur">
        ...
        <button onClick={() => changeMode('mu')} className={`inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${mode === 'mu' ? 'bg-violet-500/15 text-violet-300 border border-violet-500/30' : 'text-slate-400 hover:text-white'}`}><Activity size={13} /> MU 研究模式</button>
        <span className="hidden lg:inline-flex shrink-0 items-center gap-1 px-2 text-[10px] text-slate-600"><Gauge size={11} /> Confirmed Entry + Rank + Live</span>
      </div>
      {mode === 'daytrade' ? <DayTradingAppV5 /> : mode === 'mu' ? <MuResearchPanel /> : <App />}
    </>
  );
}
