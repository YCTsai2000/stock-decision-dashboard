  ...
  return (
    <>
      <div className="fixed top-2 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-1 rounded-xl border border-slate-700/80 bg-slate-950/95 p-1 shadow-2xl backdrop-blur">
        ...
        <button onClick={() => changeMode('daytrade')} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${mode === 'daytrade' ? 'bg-cyan-500/15 text-cyan-300 border border-cyan-500/30' : 'text-slate-400 hover:text-white'}`}><CandlestickChart size={13} /> 當沖模式 V6.3.4</button>
