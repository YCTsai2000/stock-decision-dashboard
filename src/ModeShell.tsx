...
import App from './App';
import DayTradingApp from './DayTradingApp';
  ...
  return (
    <>
      <div className="fixed top-2 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-1 rounded-xl border border-slate-700/80 bg-slate-950/95 p-1 shadow-2xl backdrop-blur">
        <button
          onClick={() => changeMode('strategic')}
          className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${mode === 'strategic' ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30' : 'text-slate-400 hover:text-white'}`}
        >
          <LineChart size={13} /> 日線決策
        </button>
        <button
        ...
        <span className="hidden sm:inline-flex items-center gap-1 px-2 text-[10px] text-slate-600"><Gauge size={11} /> Dual Engine</span>
      </div>
      {mode === 'daytrade' ? <DayTradingApp /> : <App />}
    </>
  );
}
