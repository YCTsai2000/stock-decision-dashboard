...
export default function DayTradingAppV4() {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-200 pt-14 pb-16"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <header className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-1 flex items-center gap-2 text-sm font-bold text-cyan-400"><Zap size={16}/> DAY TRADING V4 · DIRECTION + RANK</div><h1 className="text-2xl font-black text-white md:text-3xl">Underlying Signal → Bull/Bear ETF Execution</h1><p className="mt-2 max-w-4xl text-xs text-slate-500">方向由原型股決定；排名同時評估 Setup 與槓桿 ETF 執行品質。技術 Stop/TP 始終由原型股即時價觸發。</p></div><div className="flex gap-2 text-xs"><Metric label="New York" value={`${clock.date} ${clock.time}`} /><Metric label="Session" value={phaseLabel(phase)} /></div></header>

    <div className="mb-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><div className="flex flex-wrap gap-2">{WATCHLIST.map(symbol => <button key={symbol} onClick={() => setActive(symbol)} className={`rounded-lg border px-3 py-2 text-xs font-bold ${active === symbol ? 'border-cyan-500 bg-cyan-500/10 text-cyan-300' : 'border-slate-700 bg-slate-950 text-slate-400'}`}>{symbol}</button>)}</div><div className="mt-3 flex flex-wrap items-center gap-2"><select value={benchmarkMode} onChange={e => setBenchmarkMode(e.target.value)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs"><option value="AUTO">AUTO: {profile.benchmark}</option>{BENCHMARKS.map(b => <option key={b}>{b}</option>)}</select><button onClick={() => void refresh()} disabled={loading} className="inline-flex items-center gap-2 rounded-lg border border-cyan-500/40 bg-cyan-500/10 px-3 py-2 text-xs text-cyan-300"><RefreshCw size={13} className={loading ? 'animate-spin' : ''}/>更新</button><span className="text-[10px] text-slate-500">{streamStatus === 'connected' ? <Wifi size={11} className="inline"/> : <WifiOff size={11} className="inline"/>} {streamText} · 監看 {streamSymbols.length} 個 ticker</span></div>{error ? <div className="mt-3 text-xs text-rose-300">{error}</div> : null}</div>
